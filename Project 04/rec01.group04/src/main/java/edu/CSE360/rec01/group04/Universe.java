 ...
 * Completion Time = 11 + 1 + 4 + 8 = 27 hours 
...
public class Universe extends JFrame implements ChangeListener {
	...
	int elapsedTime = 0;
	Timer currentTimer;
	public static CompanionPanel companionPanel;
	ControlCenter cc;
	...
	public Universe() {
		...
		centralPanel.add(companionPanel);
		centralPanel.add(tutorPanel);
		centralPanel.add(assessor.assessorScrollPane);
		centralPanel.add(fourthPanel);
		companion = new BasicCompanion();
		
		cc.addObserver(companion);
...
//		this.stopTimer();
	...
	private void initializeSlider() {
		...
		js = new JSlider(JSlider.HORIZONTAL, 0, cc.getSliderCount(), 0);
	...
	public void stateChanged(ChangeEvent e) {
		companionPanel.stopAnimation();

		stopTimer();
		
		sliderValue = js.getValue();
		cc.setIsDefaultSliderState(sliderValue);
		tutorPanel.changeState(sliderValue);
		companion = new BasicCompanion();
		cc.addObserver(companion);
		companionPanel.setCompanion(companion);
		companionPanel.displayCompanion();
